            ...
            <table className="w-full text-[13px]">
              <thead>
                <tr className="text-slate-500 text-[11px] uppercase tracking-wide">
                  ...
                  <th className="text-left font-semibold px-2 py-2 w-24">{t('items.colUnit')}</th>
              ...
              <tbody>
                {items.map((r, i) => (
                  <tr key={i}>
                    ...
                    <td className="px-2 py-1"><input className={cell} value={r.unit} onChange={(e) => setCell(i, 'unit', e.target.value)} /></td>
                    ...
                    <td className="px-2 py-1 text-center">
                      <button type="button" className="text-red-400 hover:text-red-600 text-lg leading-none" onClick={() => removeRow(i)} aria-label="remove">×</button>
                    </td>
                  </tr>
                ))}
                {items.length === 0 && (
                  <tr><td colSpan={8} className="text-center text-slate-400 py-4 text-[13px]">{t('items.none')}</td></tr>
                )}
              </tbody>
            </table>
          </div>
