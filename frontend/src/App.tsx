...
function Layout({ children }: { children: ReactNode }) {
  ...
  const initial = (user?.name || 'L').trim().charAt(0).toUpperCase();
  const now = new Date();
  const todayStr = `${now.toLocaleDateString('th-TH', { weekday: 'long' }).replace('วัน', '')}, ${now.toLocaleDateString('th-TH', { day: 'numeric', month: 'long' })} ${now.getFullYear()}`;
  return (
    <div className="min-h-screen bg-[linear-gradient(150deg,#f7f4ec_0%,#f2f5f0_45%,#e9f3ee_100%)] lg:grid lg:grid-cols-[252px_1fr]">
      ...
      <main className="min-w-0 p-4 sm:p-6 lg:p-7 lg:max-h-screen lg:overflow-y-auto">
        <div className="flex items-center justify-between gap-3 mb-5 flex-wrap">
          <div className="rounded-full bg-gradient-to-r from-[#34d399] to-[#10b981] px-5 py-2.5 text-white text-[14px] sm:text-[16px] font-extrabold shadow-sm whitespace-nowrap">{todayStr}</div>
